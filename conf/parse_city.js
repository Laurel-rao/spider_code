...
(function () {
            ...
            okCallBack: function () {
              ...
              dhtmlx.createWin({
                ...
                okCallBack: function () {
                  ...
                  $.ajax({
                    url: ctx + "confirmPassenger/report", type: "post", async: false, success: function (cw) {
  ...
  function h() {
    ...
    if (cr.length == 0 || tickets_info.length == cr.length / 2) {
      ...
      dhtmlx.alert({
        title: "警告", ok: "确定", text: "您还有未选座的乘客，请选座完成后再提交订单！", type: "alert-error", callback: function () {
        }
      })
    }
  }
        ...
        okCallBack: function () {
          ...
          dhtmlx.createWin({
            ...
            okCallBack: function () {
              ...
              $.ajax({
                url: ctx + "confirmPassenger/report", type: "post", async: false, success: function (cu) {
                  dhtmlx.modalbox.hide(ct);
                  if (cu.data == "Y") {
                    dhtmlx.alert({title: "提示", ok: messages["button.ok"], text: "举报成功", type: "alert-info"})
                  } else {
                    dhtmlx.alert({title: "提示", ok: messages["button.ok"], text: "举报失败", type: "alert-error"})
                  }
  ...
  function S(cE, cx) {
    ...
    $.ajax({
      type: "post", url: ctx + "leftTicket/submitOrderRequest", data: cv, async: false, success: function (cF) {
        if (cF.status) {
          if (cF.data == "Y") {
            dhtmlx.alert({
              title: "温馨提示",
  ...
  $("#add-train").click(function () {
    ...
    if (cv.length == 0 || cv == "请输入") {
      ...
      if (cu < 6) {
        ...
        if (!cs.test(cv)) {
          dhtmlx.alert({
            title: "输入车次", ok: "确定", text: "车次格式输入错误！", type: "alert-error", callback: function () {
              ccInputSelected = true;
              $("#inp-train").select()
            }
          })
          ...
          if (ct.test(cv) && cv.length > 4) {
            ...
            if (cv.length < 2) {
              ...
              if ($.inArray(cv, ccSelected) < 0) {
                ...
                dhtmlx.alert({
                  title: "输入车次", ok: "确定", text: "此车次已经添加过！", type: "alert-error", callback: function () {
                    ccInputSelected = true;
                    $("#inp-train").select()
                  }
                })
...
jQuery.extend({
  pop_secretStr: "", pop_start_time: "", popup_isPop: true, popup_show_login_error: function (a) {
    ...
    $.ajax({
      url: popup_url.login,
      data: {"loginUserDTO.user_name": $("#J-userName").val(), "userDTO.password": $("#J-password").val(), randCode: a},
      type: "POST",
      timeout: 10000,
      success: function (d) {
        var e = d.data;
    ...
    var b = $("#J-code-error-mask"), d = $("#J-code-error"), a = $("#J-login-code-con"), e = $("#J-login-code-success");
    if (c == 0) {
      b.hide();
      d.hide()
    } else {
